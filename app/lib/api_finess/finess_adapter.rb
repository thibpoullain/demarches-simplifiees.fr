...
class APIFiness::FinessAdapter
  ...
  def data_source
    @data_source ||= JSON.parse(APIFiness::API.get_finess(@id), symbolize_names: true)
  end

  def schemer
    @schemer ||= JSONSchemer.schema(Rails.root.join('app/schemas/finess.json'))
  end
end
